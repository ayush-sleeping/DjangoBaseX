...
Adapted from LeapDesk's `PLUGIN_ARCHITECTURE.md`, which runs four plugins in four repos with outside
...
| # | Rule | Example |
|---|------|---------|
| 1 | **Explicit app label** — `label = "qmas"` in `AppConfig` | Django derives it from the package path otherwise, and it is baked into every migration |
| 2 | **Table prefix** — every `Meta.db_table` starts `<name>_` | `qmas_quotes` |
| 3 | **Permission prefix** — `<name>.<feature>.<action>` | `qmas.quotes.create` — never a bare `view` or `create` |
| 4 | **Route prefix** — `/api/<name>/`, url names `<name>:` | `/api/qmas/quotes/` |
| 5 | **No cross-plugin imports** — `from djx_other import …` is forbidden | see below |

### Rule 5 in practice

LeapDesk has held this to **zero violations across four plugins**. When plugin A genuinely needs
...
LeapDesk's pattern, which is what lets an external admin work on a plugin without being able to
break production:
...
cd backend/plugins/qmas                        # work in your plugin
git checkout main && git pull
# ... edit, commit, push — this is YOUR repo ...

cd ../../..                                    # back in core
git add backend/plugins/qmas
git commit -m "chore(plugins): bump qmas to <short-sha>"
```
