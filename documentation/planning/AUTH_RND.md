...
**`core-fastapi-nextjs` chose JWT in `httpOnly` cookies** — access/refresh pair, refresh-token
rotation **with reuse detection**, transparent single-retry refresh on the client
([its ADR-0006](https://github.com/Leapswitch-Networks/core-fastapi-nextjs)). Note the shape: it took
...
**LeapDesk uses Laravel Fortify + Sanctum** — session-based for the first-party frontend, tokens for
...
exactly LeapDesk's arrangement, and it is a perfectly respectable end state.
