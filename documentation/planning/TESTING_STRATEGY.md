...
| No plugin imports another plugin | LeapDesk verifies by grep; we automate it |

`core-fastapi-nextjs` records the cost of not having them: `npm run build` was broken by a type
...
Adopted from `core-fastapi-nextjs`, including the reasoning:
