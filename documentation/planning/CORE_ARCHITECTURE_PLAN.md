...
|--------|--------|------------------------|
| [`core-fastapi-nextjs`](https://github.com/Leapswitch-Networks/core-fastapi-nextjs) | **Reusability** — copy it, rename it, keep taking updates | `core/` vs `project/` split enforced by tests · a registry seam · a SHA manifest + drift doctor · clone-and-re-origin so `git merge core/main` works · a setup script that renames and re-secrets |
| [`LeapDesk`](https://github.com/Leapswitch-Networks/LeapDesk) | **Parallel teams** — four modules, four repos, outside contributors | One repo per plugin as a git submodule · auto-discovery through the framework's own app mechanism · plugin-prefixed permissions/tables/routes · **no cross-plugin imports** · a sandbox branch per external developer |

**What Django gives us for free that Laravel did not.** LeapDesk needs `internachi/modular` and
...
**What we must not copy.** LeapDesk's docs live only in the core repo
([its ADR-0041](https://github.com/Leapswitch-Networks/LeapDesk)) and its plugins carry no markdown.
...
> ever knowing the word `qmas`.

This is lifted directly from both sources. In `core-fastapi-nextjs` it is `core/registry.py`, whose
docstring records the measurement that motivated it — five places where the product reached into the
platform, three of them one big literal each. In LeapDesk it is `McpBranchRegistry`, whose comment
says it exactly: *"Nothing else in core learns the word QMAS from this — the registry only ever
...
```python
# djx_qmas/apps.py — the whole integration surface of a plugin
from django.apps import AppConfig
from core import registry

class QmasConfig(AppConfig):
    name = "djx_qmas"
    label = "qmas"                    # REQUIRED and explicit — see § 3

    def ready(self):
        from . import permissions, nav
        registry.permissions.register(permissions.GROUP)
        registry.navigation.register(nav.SECTION)
        registry.api_routes.register(prefix="qmas", urlconf="djx_qmas.urls")
```
...
| Thing | Rule | Example |
|-------|------|---------|
| Django app label | `label = "<name>"`, set **explicitly** | `label = "qmas"` |
| Database tables | `Meta.db_table` starts `<name>_` | `qmas_quotes` |
| Permissions | `<name>.<feature>.<action>` | `qmas.quotes.create` |
| API routes | mounted under `/api/<name>/`, url names `<name>:` | `/api/qmas/quotes/` |
| Frontend routes | under `/(plugins)/<name>/` | `/qmas/quotes` |
...
way LeapDesk verifies it by grep — except automatically, on every run.
...
segment. This is the same shape as LeapDesk's Inertia module registry in `resources/js/app.tsx`.
...
Adopted from LeapDesk's sandbox-branch pattern, which is what lets an outside admin work on Presales
